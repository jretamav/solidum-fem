...
Dataclass paralela a `ElementState` (ADR 0010 §4), específica de la discontinuidad. Vive en `fenix/core/discontinuity_state.py`:
...
- **Stress locking**. La fase 3 del ADR 0010 valida numéricamente que `l_d = (A_e/h)·cos(θ−α)` evita el locking que aparece con `l_d = A_e/h` ingenuo. El test compara las dos versiones sobre un campo de tracción uniforme con `Γ_d` oblicua.
...
- Estado de la discontinuidad: `fenix/core/discontinuity_state.py` (`DiscontinuityState`)
