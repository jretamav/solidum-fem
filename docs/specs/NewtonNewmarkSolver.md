...
- **2026-09-22** · Auditoría global: un ensamblaje por iteración: la convergencia se evalúa al inicio de la iteración con (‖R(U_k)‖, ‖δU_{k−1}‖) y el estado trial que se comitea es el del propio ensamblaje en U_k (antes cada iteración ensamblaba dos veces para la misma secuencia de decisiones). `max_iter` es el número máximo de resoluciones por paso. `g` se impone en u₀ **antes** de ensamblar F_int(u₀) (la aceleración inicial era inconsistente con apoyos prescritos no nulos: ü₀ = 0 en vez de K(g₀ − u₀)/M). La incógnita reducida se reconstruye con `T` (`ü = T·ü_free`), así que las restricciones lineales con maestros funcionan (antes el esclavo quedaba congelado y el Newton divergía). `prepare_all_steps` por paso (hook ADR 0010 §5, antes ausente en dinámica). Masa reducida singular → `ValueError` accionable.
...
- **2026-09-22** · ADR 0015: el bucle de Newton pasa al corrector compartido `NewtonCorrector`; este solver aporta su problema por paso (`_DynamicNewtonProblem`: iterado `(u, u̇, ü, ü_libre)`, residuo dinámico inyectado, jacobiano `M + γΔt·C + βΔt²·K_t`, norma `‖TᵀR‖`; retirado `_armijo_step_dynamic`) y conserva su control de paso. Sin cambio de formulación ni de resultados (suite completa sin tocar ningún valor esperado).
