...
- **Refactor del padre**: la fase 3 (`NewmarkSolver`) sigue intacta. La subclase no extrae métodos del padre por ahora (el flujo no lineal es lo suficientemente distinto como para que la herencia sea estructural, no compartición de pasos internos). Si emergen variantes adicionales (HHT-α, generalized-α) y se ve que comparten *este* flujo no lineal específico, entonces se refactoriza al patrón con `_solve_step` virtual.
