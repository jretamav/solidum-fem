...
- **Validación externa**: [`tests/validation/test_off_axis_orthotropic.py`](../../tests/validation/test_off_axis_orthotropic.py) — 20 tests + 221 subtests contra las formas cerradas de **Jones (1999) §2.8**: módulo aparente `E_x(θ)` (ec. 2.85, 1e-12 rel.), Poisson aparente `ν_xy(θ)` (ec. 2.86, 1e-10) y coeficiente de influencia mutua `η_xy,x(θ)`. Incluye consistencia FEM sobre un `Quad4` real (1e-10), no sólo sobre la constitutiva. Cifra citable: a 45° la rigidez cae **por debajo del 10 % de E₁**, muy por debajo de lo que sugiere `E₁/E₂ = 18.75`, porque el término cruzado `(1/G₁₂ − 2ν₁₂/E₁)` domina la zona intermedia. **Mutation test documentado**: quitar el factor 2 del bloque cortante de `T` produce 39 fallos — el benchmark no pasa por construcción. **Ampliado 2026-09-10**: las tres constantes se miden ahora sobre modelos FEM de los **cinco** elementos sólidos 2D (Quad4, Tri3, Quad8, Quad9, Tri6), no sólo sobre `Quad4`; en particular `η_xy,x` medido sobre el modelo valida la recuperación del cortante engineering en todo el pipeline. El mutation test pasa de 39 a **102 fallos**.
- **Patch test ortótropo**: [`tests/test_patch_orthotropic_2d.py`](../../tests/test_patch_orthotropic_2d.py) — 11 tests + 40 subtests. MacNeal-Harder sobre malla distorsionada con los cinco elementos y θ ∈ {0°, 30°, 45°, 90°}, verificando ε **y σ** en cada punto de Gauss vía `compute_gauss_state()`. Con material isótropo `C[0,2] = C[1,2] = 0` enmascara errores de acoplamiento; con la constitutiva llena esos caminos quedan ejercitados. *Alcance acotado*: compara σ contra la misma `C` que usa el elemento, así que valida el **elemento**, no la constitutiva — verificado por mutación (caveat en `tests/validation/README.md` §11).
- **Integración**: [`tests/test_orthotropic_integration.py`](../../tests/test_orthotropic_integration.py) — 21 tests + 52 subtests. Cubre los caminos externos que ningún otro frente tocaba: regresión de la tabla publicada en el README del ejemplo (siete orientaciones vía `run_yaml`), matriz de masa con `density` (la masa no debe ver la anisotropía), validación por YAML del criterio ortótropo, degeneración isótropa end-to-end contra `Elastic2D`, y el contrato genérico del material —incluida la periodicidad θ ≡ θ+180°— que fija lo que la migración a orientación-en-el-elemento del ADR 0013 §2 debe preservar.
- Suite global 1176 → 1220 → **1259**. Frente ortótropo: **78 tests + 370 subtests** repartidos en cuatro archivos (verificación, validación externa, patch test, integración).
