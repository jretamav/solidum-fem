...
Solidum resuelve hoy **estática lineal y no lineal** (material y geométrica) sobre **estructuras 1D (truss/cable/frame 2D y 3D)**, **sólidos 2D (Quad4/Quad8/Quad9/Tri3/Tri6)** y **sólidos 3D lineales + cuadráticos (Hex8/Hex20/Hex27/Tet4/Tet10)**, con un catálogo de materiales que cubre **elasticidad, plasticidad J2 (1D/2D/3D), Drucker-Prager (2D/3D), daño isótropo (1D/2D/3D) y cohesivo traction-jump**. Sobre la misma maquinaria está abierta la línea **dinámica** completa: modal, transitorio implícito (Newmark/HHT lineal y no lineal), transitorio explícito (diferencias centradas), armónico en frecuencia y espectro sísmico. Desde la **Etapa 8** (2026-08-25) resuelve además **conducción de calor** —estacionaria y transitoria, 2D y 3D— sobre la misma infraestructura, sin acoplamiento con el campo mecánico. **1510 tests verdes + 9 skipped intencionales**. Desde la Etapa 8, sin capacidad física nueva, la línea de trabajo ha sido de arquitectura y rendimiento: ensamblaje por lotes (ADR 0014), corrector de Newton compartido (ADR 0015), estudio del acoplamiento termomecánico (ADR 0016), backends algebraicos multihilo e iterativo (ADR 0017, 0018) y red de seguridad del análisis estático (ADR 0019).
...
- ~~**I. Solver iterativo (CG precondicionado).**~~ ✅ **Implementada el mismo 2026-09-23** en el [ADR 0018](adr/0018-solver-iterativo-krylov-amg.md), tras objetar el usuario que los programas comerciales ofrecen varios solvers algebraicos. Los tres obstáculos del diferimiento cayeron (familia CG/MINRES/directo; `pyamg` compila con Visual Studio una vez corregido el `PATH`; el Newton inexacto no aplica con tolerancia fija y estricta). Se conserva abajo el análisis del diferimiento como registro; las cifras definitivas están en el ADR.

  *Registro del diferimiento (superado):* evaluada **con medición** el 2026-09-23, tras el [ADR 0017](adr/0017-backend-algebraico-pardiso.md); diferida hasta que existiera un modelo que no cupiera en RAM.
  - *Tiempo*: sobre `Hex8 n³` con apoyos reales, CG sin precondicionar contra Pardiso — 26k DOF: 1,15 s vs 0,68 s; 86k: 4,5 vs 4,1; **202k: 12,4 vs 18,7**. El cruce está en torno a los 10⁵ DOF; por debajo el directo gana. Jacobi no aporta nada (iguala o empeora, medido).
  - *Memoria — el argumento real*: pico del directo frente a `K`, ×14 (26k DOF, 313 MB), ×20 (86k, 1,46 GB), **×25 (202k, 4,43 GB)** y subiendo; CG queda en ~×1,05 (187 MB a 202k). A 5·10⁵ DOF Pardiso pediría ~15 GB. **El iterativo no es para ir más rápido: es para que el modelo quepa.**
  - *Tres complicaciones que fijan el alcance real*: (1) CG sólo sirve para SPD — cubre `LinearSolver`, `NonlinearSolver` mientras no degrade, la familia Newmark y θ-method, pero **no** el arc-length (`K` indefinida → MINRES) ni Drucker-Prager no asociado (no simétrica → GMRES/BiCGSTAB): es una familia de tres métodos con un despachador, no un solver. (2) El precondicionador que sí escala en elasticidad es **AMG**, y `pyamg` tiene wheels para Python 3.10–3.12 (el CI iría bien) pero **no para 3.14 en Windows** — el mismo muro que `scikit-sparse`; no se pudo medir en la máquina del usuario. (3) Dentro de un Newton, la tolerancia del sistema interno se acopla con la del residuo externo (**Newton inexacto**, Eisenstat–Walker): cambia el método y es decisión de formulación del usuario, no de implementación.
  - *Cuándo retomar*: con un caso real por encima de ~2·10⁵ DOF (hoy no existe: falta el importador gmsh 3D), con AMG instalable y con la política de Newton inexacto decidida. Entra como componente propio con spec, enchufado en `NewtonCorrector.solve` por el despachador del ADR 0003.

**Estado de la bifurcación**: cerradas **A**, **C** (en su núcleo mínimo) y **D**; quedan **B** y **E** como candidatas a Etapa 9, más la continuación de la línea térmica que la propia Etapa 8 habilita. Fuera de esa bifurcación original quedan registradas **G** (IGA, diferida sin fecha), **H** (apoyos elásticos, descartada como etapa propia) e **I** (solver iterativo, implementada en el ADR 0018). La decisión es del usuario y se toma con la dirección que quiera dar al proyecto; ninguna está bloqueada técnicamente.
...
> 📌 **El acoplamiento termomecánico tiene ya su estrategia decidida**: [ADR 0016](adr/0016-acoplamiento-termomecanico.md) (2026-09-22), estudio de viabilidad **sin código**. Confirma el criterio anticipado el 2026-09-10 —formularlo como **deformación propia genérica** `ε₀` (`σ = C:(ε − ε₀)`), con la térmica como caso particular `ε₀ = α·ΔT`, porque la **higroscópica** es formalmente idéntica y **domina dos órdenes de magnitud** en madera y bambú— y añade lo que faltaba por decidir: **acoplamiento débil unidireccional primero** (el monolítico como extensión aditiva), `ε₀` almacenada **por punto de Gauss** y **sin familia de elementos acoplados**. La verificación previa del ADR midió que la infraestructura ya admite nodos con `u` y `T` simultáneos —numeración, ensamblaje en una sola `K`, BCs mixtas, masa y capacidad por el mismo operador, pipeline estático completo y VTK—, de modo que **el acoplamiento débil no requiere solver nuevo**. Argumentario completo en el ADR y en [`docs/STATUS.md`](STATUS.md) §"Próximo hito".
...
- **Análisis termomecánico acoplado** — **desbloqueado** por la Etapa 8, que entregó el campo térmico sin acoplar, y con **estrategia ya decidida** en el [ADR 0016](adr/0016-acoplamiento-termomecanico.md): débil unidireccional primero, deformación propia genérica `ε₀` por punto de Gauss, sin elementos acoplados nuevos. Deja de ser una decisión pendiente y pasa a ser una etapa de trabajo acotada (pasos 1–3 del ADR: canal `ε₀` en el contrato `Material` con default cero, proveedor térmico, validación analítica).
