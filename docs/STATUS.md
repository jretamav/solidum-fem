...
| Indicador | Valor |
|---|---|
| **Tests** | 687 pasan / 5 skipped / 0 fallos (692 colectados, +20 subtests) — Fase B validación 2026-05-19: Tanda 1 (8 benchmarks: 3 DP2D + 2 Bathe-Bolourchi Frame2DEulerCorot + 3 Cook's membrane Quad8/Quad9 + refinamiento monótono), Tanda 2 (14 benchmarks: 2 Cook's Tri6, 4 ResponseSpectrumSolver Laplaciano-1D analítico, 3 HHTSolver matriz de amplificación analítica, 5 CentralDifferenceSolver CFL = 2/ω_max), Tanda 3 (2 tests: snap-through von Mises 2-bar), Tanda 4 (3 tests: locking volumétrico ν → 0.5), Tanda 5 (6 tests: internos del NonlinearSolver), Tanda 6 (18 tests: consistent tangent FD + energía Newmark + YAML e2e) y Tanda 7 (34 tests: verificaciones canónicas — patch tests Q8/Q9/Tri6, rigid body modes para Q4/Q8/Q9/Tri3/Tri6 + corot truss/frame, invariancia rotacional de Elastic2D/IsotropicDamage2D/VonMises2D, consistencia termodinámica de J2/DP/daño/cohesivo) |
...
*Última actualización: 2026-05-19 — Tanda 7: 34 tests en cuatro bloques de verificaciones canónicas. **B.1 Patch tests Q8/Q9/Tri6** (`test_patch_higher_order.py`, 3 tests): malla 2×2 (Q8/Q9) o 4 triángulos (Tri6) con corner central irregular y midnodes desplazados; campo lineal prescrito en frontera, los nodos interiores deben recuperar exactamente el campo. Completa la familia (Q4/Tri3 ya tenían patch test). **B.2 Rigid body modes explícitos** (`test_rigid_body_modes.py`, 19 tests): traslación rígida y rotación rígida infinitesimal para Q4/Q8/Q9/Tri3/Tri6 producen ε=0 y F_int=0; K elemental tiene exactamente 3 modos nulos. Para corot (Truss2DCorot, Frame2DEulerCorot) se verifica adicionalmente rotación rígida grande (90°). **B.3 Invariancia rotacional** (`test_constitutive_rotational_invariance.py`, 5 tests): para constitutivos isótropos `σ(T_ε·ε) = T_σ·σ(ε)` con θ ∈ {30°, 45°, 60°, 90°}. Cubre Elastic2D plane strain/stress, IsotropicDamage2D en carga activa y VonMises2D plane strain/stress en régimen plástico. **B.4 Consistencia termodinámica** (`test_thermodynamic_consistency.py`, 7 tests): monotonicidad de variables internas irreversibles (α plástica, κ y ω de daño) bajo carga monótona; positividad de σ:Δε_p en J2 asociada (plane strain + plane stress). Suite 653 → 687. Queda abierto un único hueco: cohesivo+embedded G_F end-to-end, bloqueado por mini-ADR previo de "solvers para softening severo".*
