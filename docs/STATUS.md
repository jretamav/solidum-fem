...
| Indicador | Valor |
|---|---|
| **Tests** | 693 pasan / 5 skipped / 0 fallos (698 colectados, +20 subtests) — Fase B validación 2026-05-19: Tanda 1 (8 benchmarks: 3 DP2D + 2 Bathe-Bolourchi Frame2DEulerCorot + 3 Cook's membrane Quad8/Quad9 + refinamiento monótono), Tanda 2 (14 benchmarks: 2 Cook's Tri6, 4 ResponseSpectrumSolver Laplaciano-1D analítico, 3 HHTSolver matriz de amplificación analítica, 5 CentralDifferenceSolver CFL = 2/ω_max), Tanda 3 (2 tests: snap-through von Mises 2-bar), Tanda 4 (3 tests: locking volumétrico ν → 0.5), Tanda 5 (6 tests: internos del NonlinearSolver), Tanda 6 (18 tests: consistent tangent FD + energía Newmark + YAML e2e), Tanda 7 (34 tests: verificaciones canónicas — patch tests Q8/Q9/Tri6, rigid body modes, invariancia rotacional, consistencia termodinámica) y Tanda 8 (6 tests: plasticidad cíclica isótropa lineal — rango elástico simétrico tras carga, tangente plástica consistente en reversal, monotonicidad y acumulación de α en ciclos completos) |
...
*Última actualización: 2026-05-19 — Tanda 8: 6 tests sobre plasticidad cíclica isótropa lineal (`test_plasticity_cyclic.py`). **Elastoplastic1D** (4 tests): rango elástico tras α_R analíticamente predicho (yield inverso en ε = α_R − (σ_y + H·α_R)/E, probado antes/después con tolerancia 1e-9); tangente plástica consistente E_t = (E·H)/(E+H) idéntica en backbone y plast inversa; monotonicidad de α en ciclo 0 → ε_R → −ε_R → 0; acumulación de α escala con amplitud del ciclo. **VonMises2D plane strain** (2 tests): monotonicidad de α y acumulación con amplitud bajo carga uniaxial cíclica en εxx. La regla de Masing **estricta** (curva post-reversal = 2·backbone) aplica a endurecimiento kinemático puro, no a isótropo lineal — el test documenta lo que sí aplica en isótropo. Suite 687 → 693. Queda abierto un único hueco: cohesivo+embedded G_F end-to-end, bloqueado por mini-ADR previo de "solvers para softening severo".*
