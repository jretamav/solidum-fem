...
| Indicador | Valor |
|---|---|
| **Tests** | 629 pasan / 5 skipped / 0 fallos (634 colectados, +20 subtests) — Fase B validación 2026-05-19: Tanda 1 (8 benchmarks: 3 DP2D + 2 Bathe-Bolourchi Frame2DEulerCorot + 3 Cook's membrane Quad8/Quad9 + refinamiento monótono), Tanda 2 (14 benchmarks: 2 Cook's Tri6, 4 ResponseSpectrumSolver Laplaciano-1D analítico, 3 HHTSolver matriz de amplificación analítica, 5 CentralDifferenceSolver CFL = 2/ω_max), Tanda 3 (2 tests: snap-through von Mises 2-bar Truss2DCorot + ArcLengthSolver punto-a-punto contra solución cerrada) y Tanda 4 (3 tests: locking volumétrico ν → 0.5 documentado para Quad4/Quad8 plane strain) |
...
*Última actualización: 2026-05-19 — Tanda 4 de Fase B (Fase D, limitación arquitectural): locking volumétrico Quad4 plane strain con ν → 0.5 blindado por test (`test_volumetric_locking.py`). Cantilever esbelto L/h=8 con barrido ν ∈ {0.3, 0.4999}: Quad4 colapsa (ratio u_y(0.4999)/u_y(0.3) < 0.5, locking severo); Quad8 mitiga (>1.5× mejor que Q4). El test no intenta corregir el locking — mitigaciones B-bar/F-bar siguen diferidas hasta caso de uso real con materiales incompresibles. Suite 626 → 629. Queda abierto un único hueco: cohesivo+embedded G_F end-to-end, bloqueado por mini-ADR previo de "solvers para softening severo".*
