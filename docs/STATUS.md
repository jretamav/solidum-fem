...
| Indicador | Valor |
|---|---|
| **Tests** | 635 pasan / 5 skipped / 0 fallos (640 colectados, +20 subtests) — Fase B validación 2026-05-19: Tanda 1 (8 benchmarks: 3 DP2D + 2 Bathe-Bolourchi Frame2DEulerCorot + 3 Cook's membrane Quad8/Quad9 + refinamiento monótono), Tanda 2 (14 benchmarks: 2 Cook's Tri6, 4 ResponseSpectrumSolver Laplaciano-1D analítico, 3 HHTSolver matriz de amplificación analítica, 5 CentralDifferenceSolver CFL = 2/ω_max), Tanda 3 (2 tests: snap-through von Mises 2-bar Truss2DCorot + ArcLengthSolver punto-a-punto contra solución cerrada), Tanda 4 (3 tests: locking volumétrico ν → 0.5 documentado para Quad4/Quad8 plane strain) y Tanda 5 (6 tests: internos del NonlinearSolver — convergencia cuadrática asintótica, line search del ADR 0011, paso adaptativo con bisección y fallo limpio sin adaptivo) |
...
*Última actualización: 2026-05-19 — Tanda 5: cobertura de internos del `NonlinearSolver` no ejercitados antes (`test_nonlinear_solver_internals.py`, 6 tests). (1) Convergencia cuadrática asintótica del Newton: shallow von Mises 2-bar corotacional bajo el punto límite muestra al menos una transición consecutiva con caída de ≥2 órdenes en el residuo (validando que la tangente devuelta por `compute_state` es exactamente `∂σ/∂ε`). (2) Line search del ADR 0011 con tres tests: `_armijo_step` hace backtracking cuando el paso completo aumenta el residuo, no afecta la respuesta convergida en problemas bien comportados, y devuelve α=1 exacto cuando `line_search=False`. (3) Paso adaptativo: bisección automática cuando el primer intento agota `max_iter`; subclase de `SolverDivergedError` lanzada con `n_bisections=0` cuando `adaptive=False`. Suite 629 → 635. Queda abierto un único hueco: cohesivo+embedded G_F end-to-end, bloqueado por mini-ADR previo de "solvers para softening severo".*
