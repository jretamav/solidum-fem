...
| Indicador | Valor |
|---|---|
| **Tests** | 653 pasan / 5 skipped / 0 fallos (658 colectados, +20 subtests) — Fase B validación 2026-05-19: Tanda 1 (8 benchmarks: 3 DP2D + 2 Bathe-Bolourchi Frame2DEulerCorot + 3 Cook's membrane Quad8/Quad9 + refinamiento monótono), Tanda 2 (14 benchmarks: 2 Cook's Tri6, 4 ResponseSpectrumSolver Laplaciano-1D analítico, 3 HHTSolver matriz de amplificación analítica, 5 CentralDifferenceSolver CFL = 2/ω_max), Tanda 3 (2 tests: snap-through von Mises 2-bar Truss2DCorot + ArcLengthSolver punto-a-punto contra solución cerrada), Tanda 4 (3 tests: locking volumétrico ν → 0.5 documentado para Quad4/Quad8 plane strain), Tanda 5 (6 tests: internos del NonlinearSolver — convergencia cuadrática asintótica, line search del ADR 0011, paso adaptativo con bisección y fallo limpio sin adaptivo) y Tanda 6 (18 tests: consistent tangent FD para VonMises2D plane strain/plane stress, DruckerPrager2D regular/apex, CohesiveDamageIsotropic linear/exponential + conservación de energía en Newmark trapezoidal con masa consistente y lumped + regresión end-to-end de los 10 ejemplos YAML de `examples/`) |
...
*Última actualización: 2026-05-19 — Tanda 6: 18 tests en tres frentes complementarios (A+B+C). **A) Consistent tangent por FD** (`test_consistent_tangent.py`, 6 tests): `VonMises2D` plane strain + plane stress, `DruckerPrager2D` rama regular + rama apex, `CohesiveDamageIsotropic` softening lineal + exponencial. Verifica `C_alg ≈ [σ(ε+δ·e_i) − σ(ε−δ·e_i)]/(2δ)` con tolerancia O(δ²) sobre estado committed plástico/dañado. Completa la familia (`IsotropicDamage1D/2D` ya tenían FD test). **B) Conservación de energía en Newmark trapezoidal** (`test_newmark_energy_conservation.py`, 2 tests): cadena truss conservativa de 3 elementos, condición inicial de desplazamiento, integración 20 periodos sin fuerza ni amortiguamiento, masa consistente y lumped — `E(t) = ½·u̇ᵀMu̇ + ½·uᵀKu` se mantiene constante con drift relativo < 1e-10. **C) Regresión end-to-end de `examples/*.yaml`** (`test_examples_yaml.py`, 10 tests): los 10 archivos YAML del directorio se ejecutan vía `fenix.run_yaml(...)` y se compara una cantidad característica del resultado contra valor cacheado (umax, ω_1, |u_history|max, envelope max). Blinda el pipeline completo (parser → dispatcher → solver → serialización) contra regresiones silenciosas. Suite 635 → 653. Queda abierto un único hueco: cohesivo+embedded G_F end-to-end, bloqueado por mini-ADR previo de "solvers para softening severo".*
