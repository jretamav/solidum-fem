...
| Indicador | Valor |
|---|---|
| **Tests** | 701 pasan / 5 skipped / 0 fallos (706 colectados, +20 subtests) — Fase B validación 2026-05-19: Tanda 1 (8 benchmarks), Tanda 2 (14 benchmarks dinámicos), Tanda 3 (2: snap-through), Tanda 4 (3: locking ν→0.5), Tanda 5 (6: internos NonlinearSolver), Tanda 6 (18: consistent tangent FD + energía Newmark + YAML e2e), Tanda 7 (34: verificaciones canónicas — patch, RBM, invariancia rotacional, consistencia termodinámica), Tanda 8 (6: plasticidad cíclica isótropa lineal) y Tanda 9 (8: profundización física — DruckerPrager2D apex/cono-abierto/regular, VonMises2D plane stress biaxial isótropo, IsotropicDamage2D trayectorias no proporcionales) |
...
*Última actualización: 2026-05-19 — Tanda 9: 8 tests de profundización física sobre materiales existentes (`test_material_deep_coverage.py`). **DruckerPrager2D** (3 tests): tracción isótropa creciente → return al apex con `√J₂(2D) = 0` exacto y α monótona; compresión isótropa creciente → cono abierto, permanece elástico (`α = 0` siempre) — documenta limitación física conocida del DP "puro" sin cap; cortante puro → activa rama regular del cono. **VonMises2D plane stress biaxial** (2 tests): yield bajo carga biaxial isótropa σxx = σyy ≈ σ_y (predicho analíticamente como `ε_yield = σ_y·(1−ν)/E` con error rel < 6%); flujo plástico mantiene simetría `ε_p_xx = ε_p_yy` y `ε_p_xy = 0`. **IsotropicDamage2D trayectorias no proporcionales** (3 tests): rotación de la dirección principal con `ε_eq` constante congela κ y ω (irreversibilidad bajo descarga relativa); trayectoria carga εxx → carga (εxx, εyy) → descarga εxx mantiene κ del máximo histórico de ε_eq; rotación + crecimiento de magnitud produce daño monótono creciente. Suite 693 → 701. Queda abierto un único hueco: cohesivo+embedded G_F end-to-end, bloqueado por mini-ADR previo de "solvers para softening severo".*
