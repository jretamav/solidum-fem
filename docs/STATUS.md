...
| Indicador | Valor |
|---|---|
| **Tests** | 702 pasan / 5 skipped / 0 fallos (707 colectados, +20 subtests) — Fase B validación 2026-05-19: Tanda 1 (8 benchmarks), Tanda 2 (14 benchmarks dinámicos), Tanda 3 (2: snap-through), Tanda 4 (3: locking ν→0.5), Tanda 5 (6: internos NonlinearSolver), Tanda 6 (18: consistent tangent FD + energía Newmark + YAML e2e), Tanda 7 (34: verificaciones canónicas), Tanda 8 (6: plasticidad cíclica), Tanda 9 (8: profundización física materiales) y Tanda 10 (1: conservación de momento lineal en CentralDifference) |
...
*Última actualización: 2026-05-19 — Tanda 10: 1 test de conservación de momento lineal en `CentralDifferenceSolver` (`test_central_difference_momentum.py`). Análogo explícito del test de conservación de energía Newmark (Tanda 6): cadena truss libre en x (sin apoyo axial, modo rígido en x) + uy fijo, velocidad inicial concentrada en el nodo central, sin fuerzas ni amortiguamiento, `Δt = 0.3·Δt_crit`. El integrador BLM conserva `P(t) = 1ᵀ·M·u̇(t)` con drift relativo < 1e-10 sobre 200 pasos. Solo lumped (CD no acepta consistente — esquema explícito requiere M diagonal, ADR 0009 fase 2). Suite 701 → 702. Queda abierto un único hueco: cohesivo+embedded G_F end-to-end, bloqueado por mini-ADR previo de "solvers para softening severo".*
