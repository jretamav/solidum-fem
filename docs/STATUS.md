...
| Indicador | Valor |
|---|---|
| **Tests** | 626 pasan / 5 skipped / 0 fallos (631 colectados, +20 subtests) — Fase B validación 2026-05-19: Tanda 1 (8 benchmarks: 3 DP2D + 2 Bathe-Bolourchi Frame2DEulerCorot + 3 Cook's membrane Quad8/Quad9 + refinamiento monótono), Tanda 2 (14 benchmarks: 2 Cook's Tri6, 4 ResponseSpectrumSolver Laplaciano-1D analítico, 3 HHTSolver matriz de amplificación analítica, 5 CentralDifferenceSolver CFL = 2/ω_max) y Tanda 3 (2 tests: snap-through von Mises 2-bar Truss2DCorot + ArcLengthSolver punto-a-punto contra solución cerrada) |
...
*Última actualización: 2026-05-19 — Tanda 3 de Fase B (validación cuantitativa): snap-through del shallow von Mises 2-bar truss (h/L=0.1) cierra el hueco "corot + arc-length cubierto sólo en sanidad" de la matriz. Cada punto convergido (u_y, λ·F_ref) trazado por `ArcLengthSolver` se compara contra la solución cerrada `P(w) = 2·E·A·(L_0 − L_d)/L_0 · (h − w)/L_d` (engineering strain, formulación Truss2DCorot) con tolerancia absoluta 0.01% de F_ref ≈ 10⁴× el residuo nodal del rtol_force = 1e-8; el segundo test garantiza que la malla de pasos cubre rama elástica + rama inestable (entorno del pico) + rama invertida (w > h, λ < 0). Suite 624 → 626. Quedan abiertos en el plan de validación dos huecos: cohesivo+embedded G_F end-to-end (bloqueado por mini-ADR de solvers para softening severo), y test de locking volumétrico ν → 0.5 (Fase D — limitación arquitectural). Truss3DCorot heredado del 2D (proyector perpendicular es plumbing); Cable2D/3DCorot no aplicables al snap-through (unilaterales, sin compresión).*
