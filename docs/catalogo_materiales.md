...
>
> **Estado interno (ADR 0014)**: todo material declara `STATE_SCHEMA = {nombre: forma}` con sus variables internas (`{}` sin historia); de ahí se deriva el almacenamiento por arreglos del ensamblaje por lotes. Los materiales de sólidos y el térmico declaran además un kernel puntual `@njit` (`BATCH_KERNEL`) que llama a su propio return mapping, de modo que el camino por lotes y el camino por elemento son la misma física.
>
