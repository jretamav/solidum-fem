...
*Última actualización: 2026-05-20 (tercera sesión, larga). **Paper JOSS refundido a formato 2024-2025** (`paper/joss/paper.md` ~1880 palabras, 21 entradas bib, 2 figuras + 1 snippet, las 6 secciones obligatorias + sub-Limitations). **Auditoría pre-submission del repo público completada**: CI GitHub Actions verde (Python 3.10/3.11/3.12 ubuntu, run #1 OK en 1m 5s), CODE_OF_CONDUCT.md por referencia al Contributor Covenant 2.1, README sin overclaim de "thermal", badge dinámico. **16 commits acumulados pusheados** a `origin/main` (incluye reestructuración del paper, set H1-H5 de mejoras, M1 + M3 + L2 opcionales aplicados, paso 1 de pre-submission). **Bloqueante crítico restante para JOSS**: historia pública ~6 semanas vs ≥6 meses que exige el journal. Esperar a octubre-noviembre 2026 manteniendo actividad pública distribuida, o aceptar riesgo de desk-reject por scope/historia. Ver `[[project_paper_joss_estado]]`, `[[project_repo_publico_decidido]]`, `[[reference_joss_requirements]]`.*
