...
\textbf{Densidad obligatoria}. El campo \texttt{density} es \textbf{requerido} al declarar un material (ADR 0008). Omitirlo lanza \texttt{TypeError} al parsear el YAML; el fallo es ruidoso e inmediato, no diferido al análisis. Si un material legítimamente carece de masa física (penalty, restricción, fixture de test), debe declararse explícitamente como \texttt{density: 0.0} — caso en el cual \texttt{gravity} emite un \texttt{WARNING} informativo señalando el material por nombre y su contribución al peso propio es nula.
