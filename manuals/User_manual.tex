...
\begin{itemize}
    ...
    \item \textbf{5 elementos finitos 2D} continuos: cuadrilátero bilineal \texttt{Quad4} y triángulo lineal \texttt{Tri3}; cuadrático serendípito \texttt{Quad8}, cuadrático Lagrangiano \texttt{Quad9} y triángulo cuadrático \texttt{Tri6}.
    \item \textbf{8 modelos constitutivos}: elasticidad lineal 1D y 2D, plasticidad J2 1D, plasticidad J2 2D en \textit{plane strain} y \textit{plane stress} (\texttt{VonMises2D}), plasticidad friccional cohesivo-friccional 2D (\texttt{DruckerPrager2D}), daño isótropo 1D y 2D con tangente algorítmica consistente, y elasticidad unilateral para cables.
    \item \textbf{6 esquemas de solución}: solver lineal directo (\texttt{LinearSolver}), Newton-Raphson incremental con paso adaptativo (\texttt{NonlinearSolver}), longitud de arco cilíndrico de Crisfield (\texttt{ArcLengthSolver}) para snap-through y snap-back, análisis modal por autovalores generalizados (\texttt{ModalSolver}), análisis dinámico transitorio lineal Newmark-$\beta$ (\texttt{NewmarkSolver}), y análisis dinámico transitorio no lineal Newton-Newmark (\texttt{NewtonNewmarkSolver}).
\end{itemize}
...
\section{Fuerzas de Cuerpo}\index{fuerza de cuerpo}\index{body force}\index{peso propio}\index{gravity}\index{densidad}

Una \emph{fuerza de cuerpo} es una fuerza por unidad de volumen $\mathbf b$ que actúa sobre todo el dominio material del elemento, no solo sobre su frontera. Cada elemento del catálogo (armaduras, cables, marcos 2D/3D, sólidos 2D) integra la contribución consistente $\int \mathbf{N}^\top \mathbf{b}\,A\,dx$ y la acumula al vector global de fuerzas. Para vigas Hermite cúbicas el reparto incluye fuerza nodal $qL/2$ y momento $\pm qL^2/12$; para barras axiales es mitad-mitad por nodo.
...
El peso propio es el caso especial $\mathbf b = \rho\,\mathbf g$. Para problemas multimaterial — donde cada elemento debe ver \emph{su propio} $\rho$ — Fenix expone un atajo declarativo: la densidad como propiedad del material, y un vector global de gravedad.
...
\textbf{Padding 2D$\leftrightarrow$3D}. Si el modelo mezcla elementos 2D y 3D, declarar el vector con tres componentes; el ensamblador pasa el slice apropiado a cada elemento según la dimensionalidad declarada por su \texttt{DOF\_NAMES}.

\textbf{Reservado para análisis dinámico}. El atributo \texttt{density} introducido aquí es la misma magnitud que consume la matriz de masa $\mathbf M_e = \int \rho\,\mathbf N^\top\mathbf N\,dV$ en futuros análisis modales y dinámicos. Declararlo ahora prepara el modelo para esas extensiones sin coste adicional (ADR 0008).
...
\subsection{Bloque \texttt{convergence}}\index{convergencia}\index{tolerancia}
...
\begin{itemize}
  ...
  \item \texttt{atol\_force\_factor} (default \texttt{1.0e-9}): factor adimensional que define el piso absoluto de fuerza. La tolerancia absoluta efectiva se autoderiva como \texttt{atol\_force\_factor $\cdot$ escala\_inicial}, donde la escala se calcula en el primer ensamblaje. Solo conviene ajustarlo si el análisis tiene tramos donde la carga característica colapsa transitoriamente y aparece falsa no-convergencia.
...
\textbf{Nota sobre el solver algebraico interno.} El sistema lineal $\mathbf{K}\,\delta\mathbf{U}=\mathbf{R}$ que aparece dentro de cada iteración se resuelve con un backend algebraico (Cholesky, LU\ldots) seleccionado \emph{automáticamente} según las propiedades de $\mathbf{K}$. Esta elección \textbf{no} es decisión de modelado y no requiere configuración. Si por motivos de diagnóstico necesitas forzar un backend específico, consulta el capítulo \emph{Anexos técnicos --- Capa algebraica} del Manual de Referencia.
...
\begin{itemize}
    \item \textbf{DOFs/nodo}: \texttt{ux, uy} \quad ($\texttt{STRAIN\_DIM} = 3$).
    \item \textbf{Cuadratura}: Gauss 3$\times$3 (9 puntos) por defecto; 2$\times$2 disponible vía \texttt{quadrature} pero con riesgo de modos espurios.
    \item \textbf{Parámetros}: \texttt{thickness}, \texttt{quadrature} (opcional).
...
\begin{itemize}
    \item \textbf{DOFs/nodo}: \texttt{ux, uy} \quad ($\texttt{STRAIN\_DIM} = 3$).
    \item \textbf{Cuadratura}: Gauss 3$\times$3.
    \item \textbf{Parámetros}: \texttt{thickness}, \texttt{quadrature} (opcional).
    \item \textbf{Diferencia con \texttt{Quad8}}: el nodo central interior añade el término $\xi^2\eta^2$ y mejora el comportamiento en problemas con campos no separables; el nodo 8 (índice central) no participa en bordes.
\end{itemize}

\subsection{Triángulo Cuadrático Completo P$_2$: \texttt{Tri6}}
...
\begin{itemize}
    \item \texttt{plane\_strain} ($\varepsilon_{zz} = 0$): return mapping radial cerrado sobre la parte desviadora 3D extendida (Simó-Hughes \S 3.3). Corrector $\Delta\gamma = f_{\text{trial}} / (2G + \tfrac{2}{3} H)$ con $N = s_{\text{trial}} / \lVert s_{\text{trial}}\rVert$ y actualización $\alpha_{\text{new}} = \alpha + \sqrt{2/3}\,\Delta\gamma$. Default histórico.
    \item \texttt{plane\_stress} ($\sigma_{zz} = 0$): \textit{plane stress projected algorithm} (Simó-Hughes \S 3.4.1). Función de fluencia proyectada $\bar f = \tfrac{1}{2}\boldsymbol\sigma^\top \mathbf P\,\boldsymbol\sigma - R^2/3$ con operador $\mathbf P = \tfrac{1}{3}\bigl[\begin{smallmatrix}2&-1&0\\-1&2&0\\0&0&6\end{smallmatrix}\bigr]$ y Newton local escalar sobre $\Delta\gamma$ en la base de autovectores de $\mathbf C_e\mathbf P$. Converge en 3--6 iteraciones por evaluación. Actualización físicamente correcta $\alpha_{\text{new}} = \alpha + \Delta\gamma\sqrt{2\boldsymbol\sigma^\top\!\mathbf P\,\boldsymbol\sigma / 3}$ (en plane stress $\Delta\gamma$ tiene unidades $[1/\text{esfuerzo}]$, por lo que la heurística $\sqrt{2/3}\,\Delta\gamma$ válida en plane strain rompe la invariancia bajo cambio de unidades). Incompresibilidad plástica cierra $e^p_{zz} = -(e^p_{xx} + e^p_{yy})$.
\end{itemize}
...
\subsection{\texttt{DruckerPrager2D} --- plasticidad friccional cohesivo-friccional 2D}\index{Drucker-Prager}\index{plasticidad friccional}
Modelo de Drucker-Prager: cono circular suave de Mohr-Coulomb con cohesión y fricción interna. Criterio de fluencia $f = \sqrt{J_2} + \eta_f\,I_1 - k(\alpha) \le 0$ con $k(\alpha) = k_0 + H\alpha$ (endurecimiento isótropo lineal en cohesión). Plasticidad \emph{no} asociada por defecto: el ángulo de dilatancia $\psi$ es parámetro independiente del ángulo de fricción $\phi$.
...
\begin{itemize}
    ...
    \item \textbf{Parámetros}: \texttt{linear\_algebra} (default \texttt{auto}; ADR 0003 \S 4).
...
\section{\texttt{ModalSolver} --- análisis modal por autovalores generalizados}\index{modal}\index{autovalores}\index{vibraci\'on}
...
\begin{nota}
La densidad es \emph{obligatoria} para todo material involucrado en análisis modal o transitorio. La omisión lanza \texttt{ValueError} (ADR 0008) listando los materiales afectados por nombre antes de iniciar el cálculo.
\end{nota}

\section{\texttt{NewmarkSolver} --- transitorio lineal Newmark-$\beta$}\index{Newmark}\index{transitorio}

Integra en el tiempo $\mathbf M\ddot{\mathbf u} + \mathbf C\dot{\mathbf u} + \mathbf K\mathbf u = \mathbf F(t)$ por la familia Newmark-$\beta$. Con $(\beta, \gamma) = (1/4, 1/2)$ --- \textit{average acceleration}, default --- es \emph{incondicionalmente estable}, sin amortiguamiento numérico, con error $\mathcal O(\Delta t^2)$.
...
\textbf{Limitaciones}: lineal ($\mathbf K, \mathbf M$ constantes; para no-linealidad material o geométrica $\to$ \texttt{NewtonNewmarkSolver}). Apoyos Dirichlet constantes en el tiempo. Paso $\Delta t$ fijo (adaptativo diferido). HHT-$\alpha$ y generalized-$\alpha$ (con amortiguamiento numérico controlado) no incluidos.

\section{\texttt{NewtonNewmarkSolver} --- transitorio no lineal Newton-Newmark}\index{Newton-Newmark}\index{plasticidad din\'amica}

Subclase de \texttt{NewmarkSolver} (Reglas \S 4 --- variante de componente existente) que añade un bucle Newton-Raphson dentro de cada paso temporal sobre el residuo dinámico:
...
\textbf{Convergencia dual} (ADR 0007): mismas tolerancias \texttt{rtol\_force} / \texttt{rtol\_disp} y factores \texttt{atol\_*\_factor} que los solvers no lineales estáticos. Tras converger cada paso se invoca \texttt{assembler.commit\_all\_states()} (trial $\to$ committed en todos los elementos); si agota \texttt{max\_iter} se lanza \texttt{RuntimeError} y se descarta el estado trial.
...
\textbf{Recuperación del caso lineal}: con materiales sin historia o no plastificados, el residuo se anula en 1 iter y el resultado coincide \emph{a paridad de bits} con \texttt{NewmarkSolver}. Validado en tests.
