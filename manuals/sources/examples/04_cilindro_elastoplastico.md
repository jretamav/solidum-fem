...
- Elegir la longitud de arco inicial en la escala del problema.
...
**Por qué longitud de arco.** El solver de Newton con control de carga ([NonlinearSolver](../../../docs/specs/NonlinearSolver.md)) impone el valor de $\lambda$ en cada paso. Cerca del colapso la curva se vuelve horizontal y más allá no hay equilibrio, así que ese esquema no puede recorrerla. El [ArcLengthSolver](../../../docs/specs/ArcLengthSolver.md) convierte $\lambda$ en una incógnita más y avanza una distancia fija, la longitud de arco, en el espacio de desplazamientos y carga. Así sigue la meseta con la carga prácticamente constante y el desplazamiento creciente. Como la meseta nunca llega a `max_lambda`, el recorrido termina al agotar `max_steps`, y el solver lo avisa.

```callout Advertencia
La longitud de arco inicial `initial_dl` es una longitud en las unidades del desplazamiento. Aquí se usa {{r:dl:.0e}} m, del orden del desplazamiento elástico del tubo. Con el valor por omisión, 0.1 m, el primer paso pide un avance del orden de mil veces ese desplazamiento: el solver aterriza en $\lambda =$ {{r:paso_excesivo.lambda:.2f}}, por encima del colapso, con un desplazamiento exterior de {{r:paso_excesivo.u_b_m:.2f}} m, {{r:paso_excesivo.u_b_sobre_elastico:.0f}} veces el elástico en el colapso. Es un equilibrio del modelo discreto sostenido por esfuerzos hidrostáticos espurios, sin significado físico. Conviene elegir `initial_dl` a partir del desplazamiento elástico esperado.
```
