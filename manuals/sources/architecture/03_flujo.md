...
Esta secuencia interna —ensamblar una vez en el iterado, evaluar la convergencia con el residuo de ese ensamblaje y el incremento que lo produjo, resolver, avanzar— es el corrector de Newton compartido (`NewtonCorrector`, ADR 0015). Los solvers iterativos no la reescriben: le entregan un objeto que describe su problema (residuo, sistema tangente, actualización) y se quedan con la estrategia del paso.
