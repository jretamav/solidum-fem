...
\begin{enumerate}
  ...
  \item Se imponen las condiciones de Dirichlet mediante el método de penalización en forma vectorizada.
  \item El subsistema algebraico resuelve el sistema lineal \texttt{K \ensuremath{\cdot} \ensuremath{\delta}U = R}. Aquí interviene el despachador (\hyperref[adr:0003]{ADR 0003}): se inspeccionan las propiedades de \texttt{K} (simetría, posible definición positiva) y se selecciona el algoritmo numérico adecuado (factorización de Cholesky cuando aplica, factorización LU en el caso general).
  \item Se evalúa la convergencia mediante un criterio dual: norma del incremento de desplazamientos y norma del residuo de fuerza, ambas en valor relativo. La convergencia del paso requiere que ambas normas se sitúen por debajo de la tolerancia.
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textit{Plásticos 1D}: \texttt{Plastic1D} con endurecimiento isótropo lineal y deformación axial escalar.
  \item \textit{Plásticos 2D}: \texttt{VonMises2D} con criterio de fluencia J2 y algoritmo de retorno radial (\textit{return mapping}) en notación de Voigt 3.
  \item \textit{Daño}: \texttt{Damage1D} y \texttt{Damage2D}, modelos de daño isótropo escalar.
...
\begin{itemize}
  \item \pendiente{Modelo de Drucker-Prager para suelos y materiales granulares.}
  \item \pendiente{Modelo de Mohr-Coulomb para suelos y rocas.}
...
Elemento de medio continuo bidimensional bajo hipótesis de tensión plana o deformación plana. Material consumido: 2D en notación de Voigt 3 (\texttt{Elastic2D}, \texttt{VonMises2D}, \texttt{IsotropicDamage2D}).
...
\hline
Triangular lineal (3 nodos) & \ensuremath{\checkmark} \texttt{Tri3} & \ensuremath{\checkmark} \texttt{Tri3} con \texttt{VonMises2D} o \texttt{IsotropicDamage2D} \\
Cuadrilátero bilineal (4 nodos) & \ensuremath{\checkmark} \texttt{Quad4} & \ensuremath{\checkmark} \texttt{Quad4} con \texttt{VonMises2D} o \texttt{IsotropicDamage2D} \\
\hline
\end{tabularx}
\end{table}
...
\begin{itemize}
  \item \pendiente{Triangular cuadrático \texttt{Tri6} (seis nodos).}
  \item \pendiente{Cuadrilátero serendípito \texttt{Quad8} (ocho nodos).}
  \item \pendiente{Cuadrilátero lagrangiano \texttt{Quad9} (nueve nodos).}
\end{itemize}
...
\begin{itemize}
  ...
  \item \pendiente{Newton-Raphson con búsqueda lineal (line-search) para mejorar la robustez ante no convergencia.}
  \item \pendiente{Solver cuasi-Newton tipo BFGS para reducir el coste de actualización de la matriz tangente.}
  \item \pendiente{Esquemas implícitos para dinámica estructural transitoria (Newmark, HHT-\ensuremath{\alpha}, Bossak-\ensuremath{\alpha}).}
  \item \pendiente{Esquemas explícitos (diferencias centradas) para problemas con propagación de ondas y contacto.}
\end{itemize}

La incorporación de un solver nuevo se realiza en \texttt{fenix/math/solver\_<nombre>.py} mediante el decorador \texttt{@SolverRegistry.register}, sin modificación del intérprete del caso, el dominio o los elementos.
...
\begin{itemize}
  \item \pendiente{Dinámica estructural transitoria implícita con integradores Newmark y HHT-\ensuremath{\alpha}.}
  \item \pendiente{Dinámica estructural transitoria explícita con diferencias centradas, condicionalmente estable.}
  \item \pendiente{Análisis modal (frecuencias propias y modos de vibración) mediante problema de autovalores generalizado.}
...
\section{Penalización vectorizada para condiciones de Dirichlet}

Las condiciones de contorno de tipo Dirichlet se imponen mediante la suma de un valor grande sobre la diagonal de los DOF restringidos. En lugar de un bucle iterativo por condición, se construye un vector disperso de penalizaciones y se suma a \texttt{K} mediante \texttt{scipy.sparse.diags}. Se evita así un bucle crítico para el rendimiento.
...
\begin{itemize}
  \item \pendiente{ADR para la introducción del análisis dinámico transitorio (esquemas implícitos y explícitos), incluyendo el contrato de la matriz de masa por elemento y el tratamiento de amortiguamiento.}
  \item \pendiente{ADR para la introducción del análisis modal (problema de autovalores generalizado), incluyendo la elección de algoritmo (Lanczos, Arnoldi).}
...
\section{Fase 2 --- Dinámica estructural transitoria}

\textbf{Estado}: \pendiente{Implementación de la fase 2 --- dinámica estructural transitoria.}

\textbf{Cambios respecto a la fase 1}:

\begin{itemize}
  \item Aparece la matriz de masa \texttt{M}, ensamblada con la misma infraestructura que \texttt{K}. Cada elemento añade un método \texttt{M\_local} consistente con su esquema de integración.
  \item Aparece (opcionalmente) la matriz de amortiguamiento \texttt{C}, típicamente como combinación de Rayleigh \texttt{C = \ensuremath{\alpha}\ensuremath{\cdot}M + \ensuremath{\beta}\ensuremath{\cdot}K}.
  \item Las variables de estado se amplían: además del desplazamiento \texttt{U}, se almacenan velocidad \texttt{Ú} y aceleración \texttt{Ü}.
  \item Se introduce una nueva familia de solvers para integración temporal: implícitos (Newmark, HHT-\ensuremath{\alpha}, Bossak-\ensuremath{\alpha}) y explícitos (diferencias centradas).
  \item El subsistema algebraico recibe matrices efectivas del tipo \texttt{K\_eff = a\ensuremath{\cdot}M + b\ensuremath{\cdot}C + c\ensuremath{\cdot}K}, donde los coeficientes dependen del integrador y del paso de tiempo. El despachador del \hyperref[adr:0003]{ADR 0003} sigue aplicando: \texttt{K\_eff} mantiene las propiedades de simetría y, a menudo, definición positiva de \texttt{K}.
\end{itemize}
...
La generalización del concepto de DOF se mantiene (siguen siendo grados de libertad mecánicos), pero \texttt{ElementState} debe ampliarse para almacenar las derivadas temporales. La semántica \textit{trial} / comprometido se generaliza al estado completo \texttt{(U, Ú, Ü, variables internas)}.
\begin{lstlisting}
...
\end{lstlisting}callout Riesgo de arquitectura
Bajo. El análisis modal reutiliza \texttt{K} y \texttt{M} ya disponibles desde la fase 2 y solo añade un nuevo tipo de algoritmo numérico.
\begin{lstlisting}
...
\end{lstlisting}
[Entrada] - [Inicializacion] - [Interprete] - [Dominio] - [Numerica: K\_m, M\_m, K\_t, C\_t, solvers] - [Salida]
                                                  |
...
\end{lstlisting}
[Entrada] - [Inicializacion] - [Interprete] - [Dominio] - [Numerica: K\_acoplada, M\_acoplada, solvers] - [Salida]
                                                  |
...
\end{lstlisting}callout Reapertura de la decisión
...
**Razones del rechazo**. El consumidor actual (FenixBAR) es local, está bajo control del mismo desarrollador y no tiene requisitos de concurrencia ni de despliegue distribuido. La interfaz pública se ha estabilizado mediante el \hyperref[adr:0002]{ADR 0002} con `SolveResult`, `fenix.run` y `fenix.run_yaml` en proceso, lo que cubre el caso de uso de consumidor externo sin la sobrecarga de un protocolo de red. La introducción de un servidor exigiría serialización de mallas y resultados, gestión de sesiones, autenticación si el servicio fuera multiusuario, y un ciclo de despliegue paralelo al del programa. Ninguna de estas necesidades existe hoy.
...
**Razones del rechazo**. La filosofía del proyecto (ver \hyperref[cap:1]{capítulo 1}) considera la transparencia y la auditabilidad del código como valor primordial. Un sistema de plugins externos abriría la puerta a componentes no auditados que podrían silenciosamente romper la convención de signos del proyecto, los contratos declarativos del catálogo o las hipótesis de validación contra soluciones analíticas. Mientras el desarrollo lo lleve un único equipo, la incorporación de cualquier componente nuevo pasa por el repositorio y por las pruebas correspondientes, lo que mantiene la trazabilidad completa.
...
\textbf{Método de penalización (para condiciones de Dirichlet)}. Estrategia de imposición de condiciones de contorno esenciales que suma un valor grande a la diagonal de los grados de libertad restringidos. Sustituye al método de eliminación o al método de multiplicadores de Lagrange. Implementación vectorizada en Fenix FEM.
...
\begin{itemize}
  ...
  \item \pendiente{Implementación de la fase 2 --- dinámica estructural transitoria.}

  \item \pendiente{Implementación de la fase 3 --- análisis modal.}

  \item \pendiente{Implementación de la fase 4 --- problema térmico.}

  \item \pendiente{Implementación de la fase 5 --- acoplamiento termo-mecánico.}
...
Compilado el 30 de abril de 2026, 12:29\\[0.5em]
Commit Git: \texttt{743f8cb} (con cambios sin confirmar)\\[1em]
