...
% (\hyperref[adr:0010]{ADR 0010}). No están en amssymb.
\usepackage{stmaryrd}
\usepackage{booktabs}
\usepackage{tabularx}
\usepackage{array}
...
El módulo \texttt{solidum/autodiscover.py} se invoca una sola vez durante la importación del paquete. Recorre, mediante \texttt{pkgutil.iter\_modules}, las carpetas canónicas (\texttt{solidum/materials}, \texttt{solidum/elements}, \texttt{solidum/math}) e importa cada módulo. Como los decoradores \texttt{@register} se ejecutan en el momento de la importación, esta operación basta para poblar todos los registros sin enumeración manual. Este mecanismo es el equivalente conceptual al \texttt{INCLUDE} automático de Fortran moderno, ejecutado a tiempo de ejecución.
...
Las tablas y reglas de cuadratura de Gauss-Legendre 1D, 2D y 3D residen centralizadas en \texttt{solidum/math/integration.py}. Cada elemento declara su \texttt{N\_INTEGRATION\_POINTS} y consume los puntos y pesos correspondientes, sin reproducción de tablas en cada subclase.
...
El usuario de Solidum no elige el solver algebraico ni tiene por qué conocer cómo falla. Un solver directo ante una matriz singular devuelve resultados absurdos sin avisar (medido: desplazamientos de miles de kilómetros). El análisis estático interpone tres capas (\hyperref[adr:0019]{ADR 0019}, \texttt{solidum/math/solvers/model\_checks.py}):
...
\hline
Material & \texttt{solidum/materials/<snake>.py} & \texttt{@MaterialRegistry.register} & No \\
Elemento & \texttt{solidum/elements/<snake>.py} & \texttt{@ElementRegistry.register} & No \\
Solver & \texttt{solidum/math/solvers/<snake>.py} & \texttt{@SolverRegistry.register} & No \\
\hline
\end{tabularx}
\end{center}
...
\begin{enumerate}
  \item \textit{Contrato por elemento.} Todos los elementos tipo armadura, cable o marco implementan \texttt{internal\_forces(U) -> ElementForces}, que devuelve las fuerzas internas en ejes locales con claves homogéneas por familia (\texttt{\{N\}} para armadura y cable; \texttt{\{N, V, M\}} para marco 2D; \texttt{\{N, Vy, Vz, T, My, Mz\}} para marco 3D). La convención de signos es la del proyecto (capítulo anterior).
...
\textbf{Decisión.} Se introduce un \textit{logger} configurable basado en el módulo estándar \texttt{logging} de Python, expuesto al resto del proyecto vía \texttt{solidum.logging.get\_logger(name)}. Todos los componentes consumen este \textit{logger} en lugar de \texttt{print}. El nivel y el formato se configuran de forma centralizada y pueden sobreescribirse desde el archivo YAML del caso o programáticamente.
...
\textbf{Decisión.} Se aplica el mismo patrón del \hyperref[adr:0006]{ADR 0006} a la convergencia, separado por criterio. La política reside en una clase pequeña \texttt{ConvergenceCriterion} en \texttt{solidum/math/convergence.py}, que encapsula configuración (cuatro tolerancias: \texttt{rtol\_force}, \texttt{rtol\_disp}, \texttt{atol\_force\_factor}, \texttt{atol\_disp\_factor}) y estado calibrado (los \texttt{atol} efectivos derivados de las escalas del primer ensamblaje). Los solvers no lineales reciben una instancia en el constructor (parámetro \texttt{convergence}), calibran al inicio de \texttt{solve()} y delegan en \texttt{evaluate()} la comparación. La semántica es AND, no OR (ambos criterios deben cumplirse simultáneamente), coherente con la convención de Bathe, Crisfield y Owen-Hinton.
...
\begin{itemize}
  ...
  \item \textbf{Fase 2} (mass lumping HRZ canónico en sólidos + nodal directo en frames). Helper centralizado \texttt{solidum.math.mass\_lumping.lump\_hrz}. Cerrada 2026-05-18 (commit \texttt{c92bf4e}).
...
\begin{itemize}
  ...
  \item \textbf{Regla D} (auditoría 2026-05-13). Disparada por \texttt{ResponseSpectrumSolver} como segundo método de cómputo sobre \texttt{ModalResult}. El algoritmo de \texttt{free\_vibration} migra a \texttt{solidum/math/modal\_response.py} junto con los nuevos \texttt{participation\_factors}, \texttt{response\_spectrum\_srss}, \texttt{response\_spectrum\_cqc}, \texttt{spectrum\_from\_sa}, \texttt{spectrum\_tabulated}. \texttt{ModalResult.free\_vibration} queda como wrapper delgado, preservando la API histórica.
\end{itemize}
...
\begin{itemize}
  \item Aparece la matriz de amortiguamiento \texttt{C = \ensuremath{\alpha}\ensuremath{\cdot}M + \ensuremath{\beta}\ensuremath{\cdot}K} (Rayleigh proporcional). Los coeficientes pueden pasarse directos o calibrarse modalmente a partir de \texttt{(\ensuremath{\xi}\ensuremath{_{1}}, \ensuremath{\omega}\ensuremath{_{1}}), (\ensuremath{\xi}\ensuremath{_{2}}, \ensuremath{\omega}\ensuremath{_{2}})} con \texttt{solidum.math.damping.rayleigh\_from\_modes}.
...
\begin{itemize}
  ...
  \item \textbf{Regla D} --- \texttt{solidum/math/modal\_response.py} agrupa todos los cómputos sobre modos: \texttt{free\_vibration} (movido desde \texttt{results.py}), \texttt{participation\_factors}, \texttt{response\_spectrum\_srss}, \texttt{response\_spectrum\_cqc}, helpers de espectros (\texttt{spectrum\_from\_sa}, \texttt{spectrum\_tabulated}). \texttt{ModalResult.free\_vibration} queda como wrapper delgado preservando la API histórica. \texttt{results.py} vuelve a su propósito declarado: dataclasses inmutables, no algoritmos.
\end{itemize}
...
La lección de arquitectura que deja la fase es que el coste real de la extensión no se concentró donde la previsión lo situaba. Lo caro no fue generalizar la infraestructura ---que ya era agnóstica al campo, aunque nadie lo hubiera comprobado--- sino la física propia del problema: la elección de los esquemas por defecto contra el principio del máximo de la ecuación de difusión, y la constatación de que la ecuación de conducción, al ser de primer orden en el tiempo, no admite reutilizar la familia de integradores de Newmark. La interfaz del material tampoco fue la anticipada \texttt{compute\_flux\_and\_tangent(grad\_T, state)}, sino \texttt{compute\_flux(grad\_T)} sin argumento de estado, porque la conducción de Fourier carece de variables internas con historia.
...
Compilado el 23 de septiembre de 2026, 09:41\\[0.5em]
Commit Git: \texttt{2cd664e}\\[1em]
