...
\textbf{Fecha}: 13 de mayo de 2026. \textbf{Estado}: aceptado; fases 1 (modal), 3 (Newmark transitorio lineal) y 4 (Newton-Newmark transitorio no lineal) implementadas.
...
La fase 1 (\texttt{ModalSolver} + ARPACK Lanczos con shift-invert), la fase 3 (\texttt{NewmarkSolver} con \texttt{(\ensuremath{\beta}, \ensuremath{\gamma})} parametrizables, factorización única de la matriz efectiva, Rayleigh proporcional y cargas por callback Python) y la fase 4 (\texttt{NewtonNewmarkSolver} --- subclase de \texttt{NewmarkSolver} que añade Newton-Raphson dentro de cada paso con jacobiano \texttt{J = M + \ensuremath{\gamma}\ensuremath{\Delta}t\ensuremath{\cdot}C + \ensuremath{\beta}\ensuremath{\Delta}t\ensuremath{^{2}}\ensuremath{\cdot}K\_t}, convergencia dual del \hyperref[adr:0007]{ADR 0007}, Rayleigh constante calibrado con \texttt{K\_0}) quedan implementadas y validadas con tests contra solución analítica y recuperación a paridad de bits del caso lineal en ausencia de plasticidad.

\textbf{Consecuencia.} El subsistema dinámico se cierra con el alcance descrito; las fases 2 (lumped), 5 (diferencias centradas), 6 (respuesta en frecuencia) y 7 (análisis espectral / sísmico modal) quedan tabuladas en la hoja de ruta del ADR sin reabrir las decisiones arquitecturales tomadas aquí. La clase \texttt{Node} conserva su semántica original.
...
**Estado**: implementado (\hyperref[adr:0009]{ADR 0009} fase 4, sesión del 2026-05-14).

**Cambios respecto a la fase 3**:

- Nueva clase `NewtonNewmarkSolver` como **variante** de `NewmarkSolver` (Reglas §4 — extensión documentada en spec corta, sin ADR nuevo porque reúsa todas las decisiones del \hyperref[adr:0009]{ADR 0009}): hereda predictores, correctores y reducción de Dirichlet; añade un bucle Newton-Raphson dentro de cada paso temporal.
- En cada paso, el residuo `R = F_ext(t_{n+1}) − F_int(u_{n+1}) − C·u̇_{n+1} − M·ü_{n+1}` se anula iterativamente con jacobiano `J = M + γΔt·C + βΔt²·K_t`, donde `K_t = ∂F_int/∂u` es la rigidez tangente corriente. Convergencia dual del \hyperref[adr:0007]{ADR 0007} calibrada al primer paso.
- Reaprovecha íntegramente la maquinaria del `NonlinearSolver` estático: `ConvergenceCriterion`, `freeze_tangent_after_iter` (Newton modificado, \hyperref[adr:0003]{ADR 0003} fase 2), `commit_all_states()` tras converger cada paso temporal — el estado interno de los elementos recupera plenamente su semántica *trial* / comprometido también en dinámica.
...
Compilado el 13 de mayo de 2026, 15:49\\[0.5em]
Commit Git: \texttt{13012c2} (con cambios sin confirmar)\\[1em]
