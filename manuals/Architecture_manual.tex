...
\section{Eliminación directa de condiciones de Dirichlet}

Las condiciones de contorno de tipo Dirichlet se imponen por eliminación directa (\hyperref[adr:0004]{ADR 0004}). Toda restricción se expresa en forma afín \texttt{u\_s = g\_s + \ensuremath{\Sigma} \ensuremath{\alpha}\_si \ensuremath{\cdot} u\_mi} y se acumula en un \texttt{ConstraintSet} (subpaquete \texttt{fenix.bc}). El ensamblador construye un operador disperso \texttt{T} que selecciona los DOF libres y un vector \texttt{g} con los términos prescritos, de modo que \texttt{u = T \ensuremath{\cdot} u\_libre + g}. El sistema entregado al subsistema algebraico es \texttt{K\_red = T\ensuremath{^{\top}}KT}, \texttt{F\_red = T\ensuremath{^{\top}}(F − K \ensuremath{\cdot} g)}. La imposición es exacta a redondeo, preserva la simetría y la positividad definida de \texttt{K}, y deja el sistema reducido apto para solvers iterativos (CG, GMRES). La forma afín cubre además, sin cambios en el solver, los casos de asentamiento prescrito y, en una fase posterior, las restricciones multipunto lineales (uniones rígidas, periodicidad, simetrías oblicuas).
...
Compilado el 30 de abril de 2026, 15:43\\[0.5em]
Commit Git: \texttt{a7ad4d5} (con cambios sin confirmar)\\[1em]
