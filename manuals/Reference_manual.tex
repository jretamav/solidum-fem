...
\textit{> \textbf{Densidad (ADR 0008)}: todos los materiales requieren el parámetro \texttt{density} (kg/m\ensuremath{^{3}} en las unidades del problema) al construir. \textbf{Obligatorio}, sin default --- omitirlo lanza \texttt{TypeError}. Valor \texttt{0.0} se admite explícitamente (fixtures, materiales de penalty/restricción) pero nunca por omisión silenciosa. Consumido por \texttt{Assembler.assemble\_self\_weight(g)} y reservado para análisis modal/dinámico.}
...
\begin{itemize}
  \item \textbf{Modelo}: daño escalar \texttt{d \ensuremath{\in} [0, 1)}, módulo secante \texttt{E\_sec = (1 − d) \ensuremath{\cdot} E}, \texttt{\ensuremath{\sigma} = E\_sec \ensuremath{\cdot} \ensuremath{\varepsilon}}.
  ...
  \item \textbf{STRAIN\_DIM}: 1 \ensuremath{\cdot} \textbf{PRIMARY\_STATE\_VAR}: \texttt{'damage'}.
  \item \textbf{Parámetros}: \texttt{E}, \texttt{kappa\_0} (umbral elástico), \texttt{alpha} (velocidad de degradación).
  \item \textbf{Variables internas}: \texttt{kappa}, \texttt{damage}.
  \item \textbf{Tangente}: secante (\texttt{E\_sec}); no consistente con tangente algorítmica del daño --- adecuado para Newton-Raphson amortiguado, no óptimo para arc-length agresivo.
  \item \textbf{Admisibilidad (ADR 0006)}: \texttt{admissibility\_scale = E \ensuremath{\cdot} \ensuremath{\kappa}\_0} --- esfuerzo umbral inicial donde se activa el daño. Garantiza invariancia bajo cambio de unidades del check \texttt{f \ensuremath{\le} atol + rtol \ensuremath{\cdot} escala}.
  \item \textbf{Compatible con}: \texttt{Truss2D}, \texttt{Truss3D}.
  \item \textbf{Referencia}: Lemaitre \& Chaboche, \textit{Mechanics of Solid Materials}; Oliver, "A consistent characteristic length for smeared cracking models" (IJNME 1989).
...
\begin{itemize}
  \item \textbf{Modelo}: extensión 2D de \texttt{IsotropicDamage1D}. Tensor constitutivo secante \texttt{C\_sec = (1 − d) \ensuremath{\cdot} C\_e}.
  ...
  \item \textbf{Evolución}: idéntica a 1D (\ensuremath{\kappa} máxima histórica + ley exponencial).
  \item \textbf{STRAIN\_DIM}: 3 \ensuremath{\cdot} \textbf{PRIMARY\_STATE\_VAR}: \texttt{'damage'}.
  \item \textbf{Parámetros}: \texttt{E}, \texttt{nu}, \texttt{kappa\_0}, \texttt{alpha}, \texttt{hypothesis} (delegado a \texttt{Elastic2D} interno).
  \item \textbf{Variables internas}: \texttt{kappa}, \texttt{damage}.
  \item \textbf{Tangente}: secante.
  ...
  \item \textbf{Limitación}: la \texttt{\ensuremath{\varepsilon}\_eq} simétrica no distingue daño en tensión vs compresión; para hormigón usar modelo de Mazars o split tensión/compresión (no implementado).
  \item \textbf{Compatible con}: \texttt{Quad4}, \texttt{Tri3}.
...
\begin{itemize}
  \item \textbf{Modelo}: J2 (Von Mises) en deformación plana; criterio \texttt{f = \ensuremath{\|}s\_trial\ensuremath{\|} − \ensuremath{\surd}(2/3)\ensuremath{\cdot}(\ensuremath{\sigma}\_y + H\ensuremath{\cdot}\ensuremath{\alpha}) \ensuremath{\le} 0}.
  \item \textbf{Algoritmo}: return mapping radial sobre la parte desviadora.
  \item Descomposición volumétrica/desviadora: \texttt{p = K\ensuremath{\cdot}tr(\ensuremath{\varepsilon})}, \texttt{s = 2G\ensuremath{\cdot}e\_dev}.
  \item Predictor elástico desviador, corrector radial \texttt{\ensuremath{\Delta}\ensuremath{\gamma} = f\_trial / (2G + ⅔\ensuremath{\cdot}H)}.
  \item Actualización: \texttt{s\_new = s\_trial − 2G\ensuremath{\cdot}\ensuremath{\Delta}\ensuremath{\gamma}\ensuremath{\cdot}N}, con \texttt{N = s\_trial / \ensuremath{\|}s\_trial\ensuremath{\|}}.
  \item Tangente algorítmica consistente derivada por linealización del return mapping.
  ...
  \item \textbf{Variables internas}: \texttt{eps\_p} (tensorial 4 componentes \texttt{[xx, yy, zz, xy]}), \texttt{alpha} (acumulada equivalente).
  \item \textbf{Admisibilidad (ADR 0006)}: \texttt{admissibility\_scale = \ensuremath{\surd}(2/3) \ensuremath{\cdot} (\ensuremath{\sigma}\_y + H \ensuremath{\cdot} \ensuremath{\alpha})} --- radio en espacio desviador de la superficie de Von Mises en el estado corriente. Es la escala natural de \texttt{\ensuremath{\|}s\_trial\ensuremath{\|}} en la frontera de fluencia J2. Por estar el return mapping en un kernel \texttt{@njit}, la tolerancia se precomputa fuera del kernel vía \texttt{self.admissibility\_tol(state\_vars)} y se pasa como \texttt{float} al JIT.
  \item \textbf{Restricción crítica}: solo deformación plana. Usar con \texttt{hypothesis='plane\_stress'} lanza \texttt{NotImplementedError} (el return mapping para plane stress requiere algoritmo diferente).
  \item \textbf{Implementación}: kernel \texttt{\_compute\_j2\_plasticity} con \texttt{@njit}.
  \item \textbf{Compatible con}: \texttt{Quad4}, \texttt{Tri3} (configurados en plane strain).
  \item \textbf{Referencia}: Simo \& Hughes, \textit{Computational Inelasticity}, cap. 3 (return mapping J2).
