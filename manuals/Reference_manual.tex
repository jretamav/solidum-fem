...
\textit{> \textbf{Densidad (ADR 0008)}: todos los materiales aceptan un parámetro opcional \texttt{density} (kg/m\ensuremath{^{3}} en las unidades del problema). Default \texttt{0.0} --- si se omite, peso propio (\texttt{gravity: [...]} en YAML) y futuras matrices de masa darán contribución nula con warning. Consumido por \texttt{Assembler.assemble\_self\_weight(g)} y reservado para análisis modal/dinámico.}
