...
- Formulaciones no estándar en módulos de usuario, fuera del programa principal (ADR 0020). El **programa principal** es Solidum estándar: elementos, materiales y solvers de elementos finitos clásicos. Una formulación de investigación que no es FEM clásico vive en un **módulo de usuario** (`solidum/user/<nombre>/`), como los elementos y materiales de usuario de FEAP, y se conecta al programa principal por piezas genéricas (registros, familias de material con sección YAML, referencias declaradas por el elemento, gancho de inicio de paso). El programa principal no conoce los módulos de usuario.
...
| **Zona gris** — clases base (`Element`, `Material`), semántica trial/commit de variables internas, esquema de `ElementState`, API pública del paquete (re-exports raíz, entrypoints `solidum.run*`, dataclasses `SolveResult`/`ModalResult`/`TransientResult`), contrato que el programa principal ofrece a los módulos de usuario (piezas P1-P8 del ADR 0020), frontera estándar / no estándar de un componente nuevo, decisiones con ADR en curso o incompleto. Lista enumerativa, no cerrada — ante la duda, el agente eleva a zona gris. | IA propone, usuario valida concepto. | Aviso conceptual breve; lectura detallada solo si el usuario la pide. |

## 4. Protocolo de presentación de cambios

- **Componente nuevo estándar (elemento / material / solver)** → el usuario abre una **spec** en `docs/specs/<Nombre>.md` a partir de la plantilla `docs/specs/_template_<kind>.md`. La spec contiene: especificación física, formulación numérica, contrato (YAML con `interface`, `parameters`, `acceptance`, `references`). La IA **no escribe código hasta que la spec está completa**; si falta algo, pregunta en la sección *Diálogo* de la spec antes de implementar. Tras validar (tests verdes contra los casos de `acceptance`), la IA actualiza `status: validated` en la spec y añade una entrada al catálogo correspondiente (`docs/catalogo_<elementos|materiales|solvers>.md`). Relación: **spec = orden de trabajo y referencia detallada por componente**; **catálogo = índice navegable** del conjunto.
- **Componente no estándar** (formulación de investigación que no es FEM clásico: discontinuidades embebidas, seguimiento de trayectoria, otra ley cohesiva, XFEM…) → va a un **módulo de usuario** (ADR 0020), con el mismo ritual de spec, `acceptance` y catálogo que un componente nuevo pero con sus propias rutas: código en `solidum/user/<nombre>/`, specs en `docs/user/<nombre>/specs/`, tests en `tests/user/<nombre>/` (con nombres de archivo propios) y entrada en la sección «Módulos de usuario» del catálogo. Carga explícita, nunca automática: `user_modules: [<nombre>]` en el YAML o `solidum.load_user_module("<nombre>")` en Python. El programa principal **sólo gana piezas genéricas**, justificadas por casos reales ajenos al módulo (§1), nunca un caso especial que nombre al módulo. El test de pureza `tests/test_main_program_purity.py` lo vigila: cada módulo nuevo registra allí sus nombres característicos. Ante la duda sobre si un componente es estándar, la IA pregunta antes de escribir código.
