...
def _is_external_dependency_missing(exc: ModuleNotFoundError) -> bool:
    ...
    Módulos opt-in de Solidum (p. ej. ``solidum.math.linalg.cholesky`` requiere
...
def _discover_package(package_name: str) -> None:
    ...
    for _, modname, _ in pkgutil.walk_packages(
        ...
        # Saltar módulos privados (p. ej. solidum.elements.solid_2d._shared).
        ...
        try:
            ...
            if _is_external_dependency_missing(exc):
                ...
                # absorbe (ver solidum.math.linalg.cholesky para el patrón).
                continue
            raise
...
def initialize() -> None:
    ...
    `solidum.math` se descubre como paquete completo: cubre `solvers/`,
